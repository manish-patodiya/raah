    ...
    <div class="container-full">
        <div class="content-header">
            <div class="d-flex align-items-center">
                <div class="me-auto">
                    <h4 class="page-title"><i class='fa fa-pencil-square-o'></i> <?php echo trans('update_template') ?></h4>
                </div>
                <a class='btn btn-sm btn-info pull-right me-1' href="<?php echo base_url('admin/notifications/notificationtemplate/add') ?>"><?php echo trans('create_template') ?></a>
        ...
        <section class="content">
            <div class="row">
                <div class='col-md-8'>
                    <div class="card">
                        <div class="card-body">
                            <div class="alert alert-danger" id='edit-temp-err' style='display:none'></div>
                            <form id='frm-edit-template'>
                                <?php echo csrf_field(); ?>
                                <input type="hidden" name='id' value='<?php echo $temp_data->id ?>'>
                                <div class="form-group">
                                    <label for=""><?php echo trans('title') ?> <span class="required"> *</span></label>
                                    <input type="text" name='title' class='form-control' value='<?php echo $temp_data->title ?>' />
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="type" class="form-control" onchange='set_text_area(this.value)'>
                                        <?php foreach ($type_arr as $key => $value) : ?>
                                            <option value="<?php echo $key ?>" <?php echo $key == $temp_data->type ? 'selected' : '' ?>>
                                ...
                                <div class='d-none' id='editor-area'>
                                    <div class="form-group">
                                        ...
                                        <input type="text" name='subject' class='form-control' value='<?php echo $temp_data->subject ?>' />
                                    </div>
                                    <div class='form-group'>
                                        <label for=""><?php echo trans('content') ?></label>
                                        <textarea name="editor_content" class="form-control" id="ckeditor"><?php echo $temp_data->type == 2 ? $temp_data->content : '' ?></textarea>
                                    </div>
                                </div>
                                <div class='d-none' id='text-area'>
                                    <div class='form-group'>
                                        <label for=""><?php echo trans('content') ?></label>
                                        <textarea name="text_content" class='form-control' rows="8"><?php echo $temp_data->type != 2 ? $temp_data->content : '' ?></textarea>
                                    </div>
                                </div>
                                <input type="hidden" name='submit' value='1' />
                                <button class='btn btn-sm btn-info pull-right' id='btn-update-temp'><?php echo trans('update') ?></button>
                            </form>
                        </div>
                    </div>
                </div>
