...
$(function () {
    ...
    $('input').keyup(function () {
        ...
        if (i == 0 && $('#frm-register').valid()) {
            $('#btn-register').addClass('btn-theme').removeClass('btn-secondary').attr('disabled', false);
        } else {
            $('#btn-register').removeClass('btn-theme').addClass('btn-secondary').attr('disabled', true);
        }
    });
        ...
        if ($('#inpt-phone').val().length == 10) {
            if ($(this).val().length == 6) {
                if (previous_otp != $(this).val() && rid) {
                    ...
                    $('#verification-err').html('Please enter the correct OTP sent to your mobile number.');
                }
            }
        } else {
...
function enable_otp_btn(action) {
    if (action) {
        $('#btn-send-otp').addClass('btn-theme').attr('disabled', false);
    } else {
        $('#btn-send-otp').removeClass('btn-theme').attr('disabled', true);
    }
}
...
function register_user() {
    $.ajax({
        beforeSend: function () {
            ...
            $("#btn-register").html(`<span class="fa-lg"><i class="fa fa-spinner fa-spin"></i></span>`);
        },
        url: base_url("/auth/sellerRegistration"),
...
function show_hide_pass(_this, inptid) {
    $(_this).toggleClass('mdi-eye mdi-eye-off');
    if ($(_this).hasClass('mdi-eye')) {
        $(_this).html(' Show');
        $(`#${inptid}`).attr('type', 'password');
    } else {
        $(`#${inptid}`).attr('type', 'text');
        $(_this).html(' Hide');
    }
}
    ...
    $("#frm-login").validate({
        ...
        submitHandler: function (form, event) {
            let login = {
                ...
                success: function (res) {
                    if (res.status == 1) {
                        window.location = base_url("seller/dashboard");
                    } else {
                        if (res.errors) {
