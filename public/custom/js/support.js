...
$(function() {
    ...
    let support_table = () => {
        $("#support_table").DataTable({
            ajax: {
                url: base_url("/admin/support/supporttickets/support_list"),
                dataSrc: 'details',
                data: function(data) {
    ...
    $(document).on('change', '.select-statu-id', function() {
        ...
        let ajax = {
            url: base_url("/admin/support/supporttickets/update_status"),
            data: {
    ...
    function reload_support_tickets() {
        $.ajax({
            url: base_url("/admin/support/supporttickets/supp_tickets_count"),
            dataType: 'json',
            success: function(res) {
                if (res.status == 1) {
                    let ids = [];
                    ...
                    if ($.inArray(1, ids) == -1) $('#panding_tickets').find('.count').html(0);
                    if ($.inArray(2, ids) == -1) $('#compl_tickets').find('.count').html(0);
                    if ($.inArray(3, ids) == -1) $('#hold_tickets').find('.count').html(0);
                    if ($.inArray(4, ids) == -1) $('#reject_tickets').find('.count').html(0);
    ...
    $("#support_detail").submit(function(e) {
        ...
        let support = {
            url: base_url("/admin/support/supporttickets/add"),
            data: formData,
            contentType: false,
            processData: false,
            method: "post",
    ...
    $(document).on('click', '.sup_delete', function() {
        ...
        swal({
            ...
            if (isConfirm) {
                $.ajax({
                    url: base_url("/admin/support/supporttickets/delete"),
                    method: "post",
                    data: {
                        ticket_id: ticket_id,
                        "csrf_test_name": $('input[name=csrf_test_name]').val(),
    ...
    $("#edit_support_detail").submit(function(e) {
        ...
        var formData = new FormData(this);
        let states = {
            url: base_url("/admin/support/supporttickets/edit"),
            data: formData,
            contentType: false,
            processData: false,
            method: "post",
    ...
    $(document).on('click', '.sup_update', function() {
        ...
        let states = {
            url: base_url("/admin/support/supporttickets/get_support_id"),
            data: {
                'ticket_id': id,
                "csrf_test_name": $('input[name=csrf_test_name]').val(),
            },
    ...
    $(document).on("click", ".description", function() {
        ...
        let support = {
            url: base_url("/admin/support/supporttickets/get_support"),
            method: "post",
            data: {
                'ticket_id': id,
                "csrf_test_name": $('input[name=csrf_test_name]').val(),
